...
&lt;!-- ATTRIBUTE GROUP </xsl:text>
    ...
    <xsl:text>
&lt;!--</xsl:text>
    <xsl:apply-templates select="descendant::xs:attribute" mode="attr-comments"/>
    <xsl:text>--&gt;
</xsl:text>
    <xsl:text>
&lt;!ENTITY % </xsl:text>
      ...
      <xsl:when test="xs:choice | xs:complexContent | xs:group |                       xs:sequence | xs:simpleContent">
        <xsl:choose>
          <xsl:when test="@mixed='true'">
            ...
            <xsl:for-each select="descendant::xs:group |                                   descendant::xs:element">
              <xsl:text>| </xsl:text>
...
--&gt;</xsl:text>
  ...
  <xsl:template match="xs:element[@name]">
    <xsl:if test="not(preceding-sibling::xs:element)">
      ...
      <xsl:apply-templates select="../xs:attributeGroup" mode="groups"/>
    </xsl:if>
    <xsl:text>
...
&lt;!ELEMENT </xsl:text>
    ...
    <xsl:text> &gt;</xsl:text>
    <xsl:if test="descendant::xs:attribute | descendant::xs:attributeGroup">
      <xsl:text>
&lt;!--</xsl:text>
      <xsl:apply-templates select="descendant::xs:attribute |   descendant::xs:attributeGroup" mode="attr-comments"/>
      <xsl:text>--&gt;
</xsl:text>
    </xsl:if>
    <xsl:if test="/xs:schema/@targetNamespace |                   descendant::xs:attribute |     descendant::xs:attributeGroup">
  ...
  xmlns CDATA #FIXED "</xsl:text>
      ...
      <xsl:if test="descendant::xs:attribute |        descendant::xs:attributeGroup">
        <xsl:apply-templates select="descendant::xs:attribute |     descendant::xs:attributeGroup" mode="attributes"/>
      </xsl:if>
      <xsl:text> &gt;</xsl:text>
    </xsl:if>
  </xsl:template>
    ...
    <xsl:variable name="sysent-name">
      <xsl:choose>
        <xsl:when test="document(@schemaLocation)/xs:schema/@id">
          <xsl:value-of select="document(@schemaLocation)/xs:schema/@id"/>
        </xsl:when>
        <xsl:otherwise>
...
&lt;!ENTITY % </xsl:text>
    ...
    <xsl:value-of select="substring-before(@schemaLocation, '.xsd')"/>
    <xsl:text>.dtd" &gt;
</xsl:text>
    <xsl:text>%</xsl:text>
    <xsl:value-of select="$sysent-name"/>
  ...
  <xsl:template match="xs:simpleType">
    <xsl:choose>
      <xsl:when test="xs:restriction/@base='xs:NMTOKEN' and                       xs:restriction/xs:enumeration">
        <xsl:text> ( </xsl:text>
      ...
      <xsl:when test="xs:list/@itemType='xs:NMTOKEN' or               xs:list/xs:simpleType/xs:restriction/@base='xs:NMTOKEN'">
        <xs:text> NMTOKENS</xs:text>
      </xsl:when>
      <xsl:when test="xs:list/@itemType='xs:IDREF' or               xs:list/xs:simpleType/xs:restriction/@base='xs:IDREF'">
  ...
  <xsl:template name="simpleType">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$type = 'xs:Name' or $type = 'xs:NCName' or                       $type = 'xs:NMTOKEN'">
        <xsl:text> NMTOKEN</xsl:text>
      </xsl:when>
      <xsl:when test="$type = 'xs:ID' or $type = 'xs:IDREF' or                       $type = 'xs:IDREFS' or $type = 'xs:ENTITY' or                       $type = 'xs:ENTITIES' or $type = 'xs:NMTOKENS'">
        <xsl:text> </xsl:text>
