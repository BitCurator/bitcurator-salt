...
if [[ $? -ne 0 ]]
then
  echo "==> Cast release failed, aborting"
  exit 1
else
  echo "==> Cast release succceeded, continuing"
fi

echo "==> Getting GitHub Release"
RELEASE_ID=`curl -H "Authorization: token ${GITHUB_TOKEN}" -H "Accept: application/vnd.github+json" -q https://api.github.com/repos/bitcurator/bitcurator-salt/releases | jq -r ".[] | select(.tag_name == \"${TAG_NAME}\") | .id"`
